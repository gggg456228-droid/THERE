...
Исходник iOS находится в `MOBILE/IOS`. Он открывает серверную версию THERE во внешнем браузере. Чтобы синхронизация персонажа появилась на iPhone, серверная версия THERE должна быть развернута с этим же кодом. Для установки нативного приложения на обычный iPhone требуется Apple подпись.
